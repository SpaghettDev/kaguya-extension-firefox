...
export default class BlogTruyen extends MangaSource {
    constructor() {
        super({
            name: "BlogTheoYeuCau",
            id: "blogtruyen",
    ...
    async loadChapters(mangaId: string): Promise<ChapterType[]> {
        const response = await fetch(`https://blogtruyen.vn/${mangaId}/`);
        const text = await response.text();
    ...
    async loadImages(
        ...
        const response = await fetch(
            `https://blogtruyen.vn/c${chapterId}/${extraData.slug}`
        );
        const text = await response.text();
